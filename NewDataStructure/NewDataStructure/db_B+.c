...
node* insert(node* root, int key, int value) {
     ...
     * value.
     */
    pointer = make_record(value); // value에 대한 레코드 만들기
...
int main(int argc, char** argv) {
    ...
    clock_t start = clock();
    while (1) {
        instruction = fgetc(fp1);
        if (instruction == EOF) break;
        fscanf(fp1, "%d", &input);


        switch (instruction) {
        case 'i':
            root = insert(root, input, input);//insert함수 뭐냐!
           // print_tree(root);// 단순히 출력 함수
            break;
        case 's':
        case 'p':
